// P.S. (vsavchyn-dev) As of Jan 29, 2026, this module was tested against [new example](../../examples/singleton_sandbox.rs),
// [old intents sandbox env implementation](https://github.com/near/intents/blob/d38a46ad777036b7217694f785b90e6de277a3a7/sandbox/src/lib.rs#L86-L108),
// and [new intents sandbox env implementation that introduces test-suite-based cleanup](https://github.com/near/intents/blob/9e45cccb322744c34ea0265d7850ff12d012f39f/sandbox/src/lib.rs#L109-L141)
//
// Manually checked if static and per-test sandbox instance cleans up on these scenarios:
//  - testing finished normally
//      - in case of per-test sandbox, Drop implementation is called. drop takes care of
//        unregistering pid and cleaning up processes.
//      - in case of static sandbox, `libc::atexit(cleanup_remaining_sandboxes)` is called. `cleanup_remaining_sandboxes` takes care of
//        cleaning up sandboxes
//  - testing ended with SIGINT to test suite
//      - in case of per-test sandbox, `SIGINT` handler in `spawn_signal_handler` is used. `kill_sandbox_processes` takes care of
//        cleaning up sandboxes puls with `.kill_on_drop(true)`
//      - in case of static sandbox, `libc::atexit(cleanup_remaining_sandboxes)` is not called and
//        `SIGINT` handler in `spawn_signal_handler` takes care of sandbox
...
///
/// This exists because `atexit` does NOT run or signal termination (POSIX defines it as abnormal
